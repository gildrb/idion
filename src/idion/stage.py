...
ROOT_PACKAGE_APPLIED_MARKER = ".adds/idion/root-package.sha256"
LEGACY_ROOT_PACKAGE_APPLIED_MARKER = ".adds/koreader-appliance/root-package.sha256"
NICKELMENU_CONFIG = """# Generated by idion.
menu_item:main:KOReader:cmd_spawn:quiet:exec /bin/sh /mnt/onboard/.adds/idion/koreader-launch.sh
"""
...
exit 1
"""
DEPLOYMENT_MARKER = ".idion.json"
LEGACY_DEPLOYMENT_MARKER = ".koreader-appliance.json"
LIBRARY_MARKER = ".adds/idion/library.json"
LEGACY_LIBRARY_MARKER = ".adds/koreader-appliance/library.json"
LEGACY_LAUNCHER_SCRIPT = ".adds/koreader-appliance/koreader-launch.sh"
...
def root_package_is_applied(mount: Path, sha256: str) -> bool:
    marker = under(mount, ROOT_PACKAGE_APPLIED_MARKER)
    if not marker.is_file():
        marker = under(mount, LEGACY_ROOT_PACKAGE_APPLIED_MARKER)
    try:
        return marker.is_file() and marker.read_text(encoding="ascii").strip() == sha256
...
def deployment_is_current(
    ...
    if not marker.is_file():
        marker = destination / LEGACY_DEPLOYMENT_MARKER
    if not marker.is_file():
        return False
    try:
...
def library_is_current(mount: Path, expected: str) -> bool:
    ...
    if not marker.is_file():
        marker = under(mount, LEGACY_LIBRARY_MARKER)
    try:
...
def _restore_library(mount: Path, books_root: Path, source: Path, expected: str) -> None:
    ...
    _remove_legacy_marker(under(mount, LEGACY_LIBRARY_MARKER))


def _remove_legacy_marker(path: Path) -> None:
...
def _migrate_root_package_marker(mount: Path, expected: str) -> None:
    marker = under(mount, ROOT_PACKAGE_APPLIED_MARKER)
    legacy = under(mount, LEGACY_ROOT_PACKAGE_APPLIED_MARKER)
    if marker.is_file():
        try:
        ...
        try:
            if legacy.read_text(encoding="ascii").strip() == expected:
                _atomic_text(marker, expected + "\n")
                _remove_legacy_marker(legacy)
        except (OSError, UnicodeError):
            pass


def _migrate_deployment_marker(
    destination: Path, expected: dict[str, object]
) -> None:
    marker = destination / DEPLOYMENT_MARKER
    legacy = destination / LEGACY_DEPLOYMENT_MARKER
    if marker.is_file():
        try:
        ...
        try:
            if json.loads(legacy.read_text(encoding="utf-8")) == expected:
                _atomic_text(
                    marker,
...
def _migrate_library_marker(mount: Path) -> None:
    marker = under(mount, LIBRARY_MARKER)
    legacy = under(mount, LEGACY_LIBRARY_MARKER)
    if marker.is_file():
        try:
            if json.loads(marker.read_text(encoding="utf-8")).get("schema") == 1:
                _remove_legacy_marker(legacy)
        except (OSError, json.JSONDecodeError):
            pass
    elif legacy.is_file():
...
def stage_koreader(
    ...
    trigger_received = trigger.is_file()
    if trigger_received and _sha256(trigger) != source_hash:
        trigger_received = False
    if not trigger_received and not root_package_is_applied(mount, source_hash):
    ...
    if trigger.is_file():
        ...
        marker = under(mount, ROOT_PACKAGE_APPLIED_MARKER)
        if not marker.is_file() or not root_package_is_applied(mount, source_hash):
            _atomic_text(marker, source_hash + "\n")
    elif not root_package_is_applied(mount, source_hash):
        raise OSError("root package trigger was not staged")
    _migrate_root_package_marker(mount, source_hash)
