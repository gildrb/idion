...
function KoboBluetooth:_cleanup(broadcast_refresh)
    ...
    -- Cancel any scheduled input-device discovery from an earlier connection
    -- or power lifecycle. Scheduled callbacks compare this generation before
    -- touching the device or UI.
    self.connection_generation = self.connection_generation + 1
...
---
--- Restores the WiFi state observed before Bluetooth setup.
--- MTK Bluetooth temporarily requires WiFi, but must not change the user's
--- deliberate network reachability choice.
...
function KoboBluetooth:_handleConnection(device_address)
    ...
    local function try_open()
        if generation ~= self.connection_generation or not device.connected then
            return
        end

        local detected_path = device.name and self.input_handler:findDeviceByName(device.name)
        if detected_path or attempts >= max_attempts then
            -- Name matching inside openIsolatedInputDevice will reuse the path.
            -- The final attempt permits its non-blocking event-device fallback.
            finish(self.input_handler:openIsolatedInputDevice(device, show_notifications, false))
            return
        end

        attempts = attempts + 1
        UIManager:scheduleIn(retry_seconds, try_open)
    end
