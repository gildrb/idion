...
function BluetoothKeyBindings:onBluetoothKeyEvent(key_code, key_value, time, device_path)
    ...
    local action_id = bindings and bindings[key_name] or nil

    -- The official Kobo Remote (N257) exposes its large/forward button as
    -- KEY_DOWN (108) and its small/back button as KEY_UP (103). Keep these
    -- usable out of the box while allowing an explicit binding to override.
    if not action_id and self.settings and self.settings.official_kobo_remote_defaults then
        local is_official_remote = false

        for _, device in ipairs(self.settings.paired_devices or {}) do
            local name = (device.name or ""):lower()
            if device.address == device_mac and name:find("kobo remote", 1, true) then
                is_official_remote = true
                break
            end
        end

        if is_official_remote and (key_code == 108 or key_code == 103) then
            local delta = key_code == 108 and 1 or -1
            logger.info("BluetoothKeyBindings: Official Kobo Remote page turn:", delta)
